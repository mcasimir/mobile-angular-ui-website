...
DocletsReader.prototype._read = function() {
  ...
  var docletsBuff = '';
  var args = ['-r', sources, '-c', JSDOC_CONF, '-X'];
  var child = spawn(JSDOC_BIN, args);
  ...
  child.stderr.on('data', function(err) {
    let msg = '[Jsdoc error] ' + err.toString('utf8');
  ...
  child.stdout.on('end', function() {
    ...
    try {
      doclets = JSON.parse(docletsBuff);
    } catch (e) {
      throw new Error(`Failed to parse: ${docletsBuff}`);
    }
